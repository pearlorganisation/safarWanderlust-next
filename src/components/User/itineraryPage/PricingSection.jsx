...
const PricingComponent = ({
  ...
  useEffect(() => {
    if (batches) {
      const arr = ["JAN", "FEB", "MAR", "APR", "MAY", "JUNE", "JULY", "AUG", "SEP", "OCT", "NOV", "DEC", "ALL"]
  ...
  return (
    <div className="h-auto ">
      <div className="lg:block px-6 min-w-full max-w-full font-titleRegular  mt-4 bg-white py-6 ">
        {/* Pricing Section - Starting from */}
        <div
          style={{ boxShadow: "rgba(0, 0, 0, 0.24) 0px 3px 8px" }}
        ...
        >
          <div>
            ...
            <div className="flex flex-wrap items-center mt-2 gap-2">
              ...
              <span className="text-sm bg-white text-gray-700 px-4 py-4 "></span>
              <span className="text-xs bg-blue-100 text-gray-700 px-2 py-1 rounded-full">Per Person</span>
            </div>
        ...
        <div className="mb-6 ">
          ...
          <div className="relative bg-[#f97613] w-full h-auto max-h-16 overflow-y-auto overflow-x-auto scrollbar-orange p-1">
            {mapToHoldBatches &&
              currMapKeySelected &&
              mapToHoldBatches?.get(currMapKeySelected)?.map((batch, index) => {
                return (
                  <div
                    ...
                    key={index}
                    className={`bg-slate-50 p-2 m-2 border-2 hover:border-red-400 rounded-md inline-block ${selectedBatch === batch ? "border-red-500" : "border-gray-300"} ${batch?.is_sold ? "cursor-not-allowed opacity-60" : ""}`}
                  >
                    <p
                      className={`${
                        selectedBatch === batch ? "bg-gray-200" : "bg-gray-50"
                      } hover:bg-gray-300 text-slate-800 cursor-pointer hover:scale-y-105   font-semibold`}
                    >
                      {moment(batch.start_date).format("DD-MMM ")} To {moment(batch.end_date).format("DD-MMM ")}
                    </p>
                  </div>
