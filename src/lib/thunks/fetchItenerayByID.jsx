...
export const fetchItenerayByID = (itenerayId) => async (dispatch) => {
  // Set loader to true
  ...
  try {
    ...
    const response = await get(
      `${API_ENDPOINTS.COMMON.GET_ITINERARY}/${itenerayId}`
    )


    if (response.message === 'ITINERARY_FOUND' && response.success === true) {


      // Extract the necessary data for the category
      ...
      dispatch(setValue({ key: 'itenerayByID', value: {...itinerary,allImages:images} }));
    }
  } catch (error) {
